...
Create, allocate and synchronize on host if source view is not in host space

\begin{minted}{c++}
auto mirrorView = Kokkos::create_mirror_view_and_copy(view);
\end{minted}
...
\begin{tblr}[theme=kokkostable]{lX}
    ...
    \mintinline{c++}{IndexType} & Integer type for the index; defaults to the one defined by \mintinline{c++}{ExecutionSpace}, usually \mintinline{c++}{int64_t} on CPUs and \mintinline{c++}{int32_t} on GPUs \\
...
\begin{tblr}[theme=kokkostable]{Xl}
    ...
    \mintinline{c++}{Kokkos::atomic_div(&x, y)} & \mintinline{c++}{x /= y} \\
    ...
    \mintinline{c++}{Kokkos::atomic_mul(&x, y)} & \mintinline{c++}{x *= y} \\
...
All C++ standard math functions are available.
...
\begin{tblr}[theme=kokkostable]{lX}
    Macro & Description \\
    \mintinline{c++}{KOKKOS_LAMBDA} & Annotations and capture argument for lambdas \\
    \mintinline{c++}{KOKKOS_CLASS_LAMBDA} & Annotations and capture argument, including \mintinline{c++}{*this}, for lambdas \\
    \mintinline{c++}{KOKKOS_FUNCTION} & Annotations for free functions or functors \\
    \mintinline{c++}{KOKKOS_INLINE_FUNCTION} & Annotations and inline specifier for free functions or functors \\
\end{tblr}

These macros add the compiler-specific annotations required by the enabled GPU backend (\mintinline{cuda}{__host__ __device__} for CUDA and HIP).
