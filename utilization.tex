...
\subsubsection{Create and always allocate on host}

\begin{minted}{c++}
auto mirrorView = Kokkos::create_mirror(view);
\end{minted}

\subsubsection{Create and allocate on host if source view is not in host space}

\begin{minted}{c++}
auto mirrorView = Kokkos::create_mirror_view(view);
\end{minted}

\subsubsection{Create, allocate and synchronize if source view is not in same space as destination view}
