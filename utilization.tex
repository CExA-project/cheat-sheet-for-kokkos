...
\subsubsection{Initialize and finalize}
...
\subsection{Kokkos concepts}

\subsubsection{Execution spaces}
...
\paragraph{Generic memory spaces}
...
\paragraph{Specific memory spaces}
...
\paragraph{Create and always allocate on host}

\begin{minted}{c++}
auto mirrorView = Kokkos::create_mirror(view);
\end{minted}

\paragraph{Create and allocate on host if source view is not in host space}

\begin{minted}{c++}
auto mirrorView = Kokkos::create_mirror_view(view);
\end{minted}

\paragraph{Create, allocate and synchronize if source view is not in same space as destination view}
...
\subsubsection{Scatter view (experimental)}

\paragraph{Specific header}
...
\paragraph{Scatter, compute, and gather}
...
\paragraph{Global fence}

\begin{minted}{c++}
Kokkos::fence("label");
\end{minted}

\paragraph{Execution space fence}

\begin{minted}{c++}
ExecutionSpace().fence("label");
\end{minted}

\paragraph{Team barrier}
...
\paragraph{Multi-dimensional (dimension 2)}
...
\paragraph{Team vector level (2-level hierarchy)}
...
\subparagraph{One-dimensional range}

\begin{minted}{c++}
Kokkos::TeamVectorRange range(teamMember, firstJ, lastJ);
\end{minted}

\subparagraph{Multi-dimensional range (dimension 2)}
...
\paragraph{Team thread vector level (3-level hierarchy)}
...
\subsubsection{Create and populate}
...
\subsubsection{Atomic exchanges}
...
\subsubsection{Code interruption}

\begin{minted}{c++}
Kokkos::abort("message");
\end{minted}

\subsubsection{Print inside a kernel}
...
\subsubsection{Manage parallel environment}

\begin{tblr}[theme=kokkostable]{lX}
    Function & Description \\
    \mintinline{c++}{Kokkos::device_id()} & Returns the device ID of the current device \\
    \mintinline{c++}{Kokkos::num_devices()} & Returns the number of devices available to the current execution space \\
\end{tblr}

\subsection{Macros}

\subsubsection{Essential macros}
