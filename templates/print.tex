...
$endif$
\PassOptionsToClass{a4paper,10pt}{article}
%
\documentclass[
$if(fontsize)$
  $fontsize$,
...
\usepackage[left=0.5cm, right=0.5cm, top=0.00cm, bottom=2.00cm, includehead]{geometry}
\usepackage{multicol}
\usepackage{mdframed}
\usepackage{fancyhdr}
\usepackage{calc}
...
\fancypagestyle{kokkos}{%
	...
	\fancyhead[R]{%
		% Add the version
		\parbox[b][\headheight][t]{\headheight}{%
			\vspace{0.5em}%
			\raggedleft%
			{v0.0.0}%
...
\newcommand{\subsubsectionstyle}[1]{%
	% The sub sub section name has a small left margin, which only affects the first line if the name spawns on multi lines; this is inelegant but I haven't found a better way for now
...
\let\oldtexttt\texttt
\renewcommand{\texttt}[1]{{\scriptsize\oldtexttt{#1}}}
